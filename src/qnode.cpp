...
//ROS_INFO("/goal_pose_topic: %s\n",string_var.c_str());
    ...
    pos_sub = n.subscribe(pose_topic,1,&QNode::poseCallback,this);
...
{
        emit velocity_yaw(Velocity_yaw->vx ,Velocity_yaw->vy,Velocity_yaw->vz,Velocity_yaw->yaw );
}
...
{
    emit velocity_setpoint( Velocity->twist.linear.x , Velocity->twist.linear.y ,Velocity->twist.linear.z ,
...
{
    emit velocity( Velocity->twist.linear.x , Velocity->twist.linear.y ,Velocity->twist.linear.z ,
 	Velocity->twist.angular.x , Velocity->twist.angular.y ,Velocity->twist.angular.z );
}
...
void QNode::accelCallback(const sensor_msgs::Imu::ConstPtr& Accel)
{
    ...
    emit accel( Accel->linear_acceleration.x , Accel->linear_acceleration.y , Accel->linear_acceleration.z);
}
